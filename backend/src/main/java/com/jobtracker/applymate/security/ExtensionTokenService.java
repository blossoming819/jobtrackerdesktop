...
public class ExtensionTokenService {
  ...
  public void require(String token) {
    ...
    boolean valid = mapper.selectCount(new LambdaQueryWrapper<ExtensionPairing>().eq(ExtensionPairing::getStatus,"APPROVED").eq(ExtensionPairing::getTokenHash, hash(token))) > 0;
